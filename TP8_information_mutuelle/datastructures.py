...
def save_xml(corpus : Corpus, output_file: Path=None) -> None:
...
def save_json(corpus: Corpus, output_file: Path = None) -> Union[None, str]:
    data = []
    for item in corpus.items:
        the_date = None
        if item.date is not None:
...
def save_pickle(corpus : Corpus, output_file: Path=None) -> None:
    if output_file is None:
        return pickle.dumps(corpus)
    else:
